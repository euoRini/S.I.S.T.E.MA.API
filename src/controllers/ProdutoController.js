...
module.exports = {
  ...
  async deleteByName(req,res)
  {
    ...
    if(!produto) return res.status(400).send('Produto não cadastrado.');
    await produto.destroy();
    return res.status(200).send('Produto removido do catálogo com sucesso!');
  },
  
  async findByName(req,res){
    const { nome } = req.params;
    const find = await Admins.findOne({ where: { nome: nome } });  
    if(!find) return res.status(400).send('Produto não encontrado.');
